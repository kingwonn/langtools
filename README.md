...
| **记忆系统** | 三层结构：① 分析时提取值得记住的**观点论断**（带维度标签：技术/行业/投资/争论）→ ② 沉淀到对应词条的**记忆时间线**（带出处与日期）→ ③ 积累足量新记忆后自动**巩固**为词条的「最新动态与争论」小节。词条随时间进化，支持后续按维度做跨概念分析 |
...
- **增量**：只分析新文章、只为新概念建词条、记忆按文章去重入账，已有内容不重复调用
- **限额**：每次运行最多分析 20 篇、新建 8 个词条、巩固 3 个词条（环境变量 `ANALYZE_MAX_PER_RUN` / `WIKI_MAX_NEW_ENTRIES` / `WIKI_MAX_SYNTHESIS_PER_RUN` 可调），成本可控
